...
export function createTasksCollection(adminBeforeListComponent?: string): CollectionConfig {
  const admin: NonNullable<CollectionConfig['admin']> = {
    useAsTitle: 'name',
  ...
  return {
    ...
    hooks: {
        ...
        if (d.type === 'periodic' && Array.isArray(d.schedule_conditions) && d.schedule_conditions.length > 0) {
          ...
          const startPoint =
            operation === 'create'
              ...
              : d.finished_at
                ? new Date(d.finished_at as string)
                : (() => {
                    const t = new Date()
                    t.setMinutes(t.getMinutes() + firstRunDelay)
                    return t
                  })()
          const conditions = d.schedule_conditions as ScheduleCondition[]
      ...
      ({ data }) => {
        const d = data as Record<string, unknown> | undefined
        if (!d) return data
        if (d.type === 'periodic' && (!d.schedule_conditions || (d.schedule_conditions as unknown[]).length === 0)) {
        ...
        if (d.type === 'periodic' && Array.isArray(d.schedule_conditions)) {
          const result = validateScheduleConditions(d.schedule_conditions as ScheduleCondition[])
          if (!result.valid) {
            throw new Error(
              `Условие расписания #${result.conditionIndex + 1}: за 7 дней не найдено подходящее время`,
            )
          }
        }
        return data
