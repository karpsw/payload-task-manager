...
import {
  getScheduleStepMinutes,
  getScheduleHorizonDays,
  getMinPeriodMinutes,
  getFirstRunDelayMinutes,
  getTimezoneOffsetHours,
} from './options.js'
import type { ScheduleCondition, ScheduleIntervalKey } from './types.js'

const MS_PER_MINUTE = 60 * 1000

/** Apply timezone offset: UTC date -> "local" date for schedule (offset in hours) */
function toLocalScheduleDate(d: Date, offsetHours: number): Date {
  const t = new Date(d)
...
function fromLocalToUtc(local: Date, offsetHours: number): Date {
  const t = new Date(local)
  t.setUTCHours(t.getUTCHours() - offsetHours)
  return t
}

function isInInterval(d: Date, key: ScheduleIntervalKey): boolean {
  const day = d.getDay()
  ...
  switch (key) {
    ...
    case 'morning-evening':
      return (
        (totalMinutes >= 5 * 60 && totalMinutes < 8 * 60) ||
...
function matchesAllIntervals(d: Date, intervals: ScheduleIntervalKey[]): boolean {
  if (intervals.length === 0) return true
  return intervals.every((key) => isInInterval(d, key))
}

/** Parse "23:15, 3:41" -> [{ hours: 23, minutes: 15 }, { hours: 3, minutes: 41 }] */
export function parseCustomTimes(s: string | undefined): { hours: number; minutes: number }[] {
...
function findNextForPeriodCondition(
  startPointLocal: Date,
  ...
  const intervals = (condition.intervals ?? []) as ScheduleIntervalKey[]

  while (candidate.getTime() <= endTime) {
    if (matchesAllIntervals(candidate, intervals)) return candidate
...
function findNextForCustomTimesCondition(
  startPointLocal: Date,
  condition: ScheduleCondition,
  horizonMs: number,
  offsetHours: number,
...
function findNextForCondition(
  ...
  const next = findNextForPeriodCondition(
    startLocal,
    condition,
    stepMs,
    horizonMs,
...
export function calculateNextRunAt(
  startPoint: Date,
  scheduleConditions: ScheduleCondition[],
): Date | null {
  ...
  for (const condition of scheduleConditions) {
    const next = findNextForCondition(
      startPoint,
      condition,
      offsetHours,
      stepMs,
 ...
 */
export function validateScheduleConditions(
  scheduleConditions: ScheduleCondition[],
): { valid: false; conditionIndex: number } | { valid: true } {
  ...
  for (let i = 0; i < scheduleConditions.length; i++) {
    ...
    if (!next) return { valid: false, conditionIndex: i }
  }
  return { valid: true }
}
